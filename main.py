...
# генерація лекцій на базі предметів
...
class CSP:
    ...
    def is_consistent(self, assignment, var, value, week_number):
        ...
        for other_var_id, other_value in assignment.items():
            ...
            if day == other_day and period == other_period:
                # Одна аудиторія одночасно
                if aud == other_aud:
                    return False
                # Один викладач одночасно
                if lect == other_lect:
                    return False
                # Одна група одночасно
        ...
        # Аудиторія має достатню місткість
        ...
        # Викладач не перевищує максимальну кількість годин
        ...
        # Специфічні обмеження (максимум занять викладача в день і тд)

        max_lecturer_daily_hours = 3  # Зараз макс 3 год на день
    ...
    def order_domain_values(self, var, assignment):
        # Евристика Least Constraining Value
...
# Функція створення доменів для кожної пари
def create_domains(lessons, lecturers, auditoriums):
    ...
    for lesson in lessons:
        ...
        if not possible_lecturers:
            continue  # Якщо не має можливих викладачів, рішення неможливе
...
# Функція для обчислення придатності на основі м’яких обмежень
...
# Функція для забезпечення дотримання обмежень щодо максимальної кількості годин викладачів на тиждень
...
if not solution:
    ...
    # Організація розкладу для друку
